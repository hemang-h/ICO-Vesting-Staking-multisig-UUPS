...
import "./Multisig/multisigV2.sol";
import "./Interfaces/IAggregator.sol";

contract Ex1ICO is Initializable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, MultisigGovernance {
    ...
    address public receivingWallet = 0x52C1ffFb760F653fe648F396747967Bb1971eb38;    
    ...
    */
    ...
    function updateRequiredNumberOfConfirmations(uint256 _requiredConfirmations) external onlyRole(OWNER_ROLE) {
    ...
    */
    function buyWithUSDC(
        ...
        IERC20(_token).safeTransferFrom(_msgSender(), receivingWallet, usdValue);
    ...
    */
    ...
    function proposeWithdraw(uint256 _amount) external onlyRole(PROPOSER_ROLE) {
        require(_amount > 0, "Invalid withdrawal amount");

        // Encode proposal data
        bytes memory proposalData = abi.encode(_amount);

        // Create proposal
        createProposal(
            uint256(ProposalType.Withdraw), 
            proposalData
        );
    }

    function executeWithdraw(uint256 _proposalId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // Check proposal is approved
        require(_checkProposalApproved(_proposalId), "Proposal not approved");

        // Decode proposal data
        uint256 withdrawAmount = abi.decode(_getProposalData(_proposalId), (uint256));

        // Perform withdrawal
        ex1Token.transfer(receivingWallet, withdrawAmount);
    }
}