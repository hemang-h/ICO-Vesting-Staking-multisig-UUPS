...
contract Ex1Staking is Initializable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    */
    ...
    function proposeUpdateEx1Token(address _tokenAddress) external onlyRole(UPGRADER_ROLE) {
        bytes memory data = abi.encode(_tokenAddress);
        bytes32 proposalHash = keccak256(data);
        proposalCount++;
    ...
    function executeProposal(
        ...
        } else if (proposal.proposalType == ProposalType.UpdateEX1Token) {
            address tokenAddress = abi.decode(proposal.paramsData, (address));
            _updateEX1TokenAddress(tokenAddress);
        }     
    ...
    */
    ...
    function _updateEX1TokenAddress(address _tokenAddress) internal {
